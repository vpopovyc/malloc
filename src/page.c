										...
										t_meta_data *md, size_t size)
{
	...
	ft_memset(&new_md, 0, sizeof(t_meta_data));
	new_md.alloc_space = size;
	new_md.prev = md;
	new_md.next = md->next;
	offset_ptr = (uint8_t*)md + md->alloc_space;
	ft_memmove(offset_ptr, &new_md, sizeof(t_meta_data));
	if (md->next)
...
void	insert_new_block_in_fresh_page(t_page *page, size_t size)
{
	...
	ft_memset(&md, 0, sizeof(md));
	md.alloc_space = size;
	ft_memmove(page->raw_data, &md, sizeof(t_meta_data));
...
t_page	*new_page(size_t size, t_page *prev_page)
{
	...
	ft_memset(&new_page, 0, sizeof(t_page));
	...
	ft_memmove(raw_mem, &new_page, sizeof(t_page));
	if (prev_page)
